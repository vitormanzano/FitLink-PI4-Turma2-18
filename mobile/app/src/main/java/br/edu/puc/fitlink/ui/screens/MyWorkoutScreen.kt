...
fun MyWorkoutsScreen(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        if (!vm.hasPersonal) EmptyState(onFindPersonal) else WorkoutList(vm)
    }
}
...
private fun EmptyState(onFindPersonal: () -> Unit) {
    Column(
        ...
        PrimaryPillButton("Procurar um Personal", onClick = onFindPersonal)
    }
}
    ...
    LazyColumn(
        ...
        vm.workoutGroups.forEach { group ->
            item {
                ...
                Text(group.title, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.ExtraBold)
